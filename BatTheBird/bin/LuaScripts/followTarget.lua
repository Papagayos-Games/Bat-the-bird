...
followTarget["instantiate"] = function (params, entity)
    ...
    self.setFollow = function (_follow)
        self.following = _follow
    end
    return self
end
...
followTarget["start"] = function (_self, lua)
    ...
    _self.targetPosition = lua:getTransform(lua:getEntity(_self.target)):getPosition()
end

followTarget["update"] = function (_self, lua)
    if _self.following and _self.target ~= nil then
        _self.transform:setPosition(Vector3(_self.position.x, _self.targetPosition.y, _self.position.z))
    end
end
